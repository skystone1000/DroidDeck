    ...
    questions: [
        {
            ...
            referenceLinks: [{ title: "Asynchronous Flow - Kotlin", url: "https://kotlinlang.org/docs/coroutines-flow.html" }, { title: "Kotlin Flow on Android", url: "https://developer.android.com/kotlin/flow" }],
...
fun main() = runBlocking {
    ...
    println("collect returned, so the ticker was cancelled")
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 <code>flowOn</code> moves everything upstream of it to another context and leaves the collector exactly where it was.</strong></p><ul><li>The docs call it <strong>context-preserving</strong>: <em>&quot;It changes only the coroutine context of the upstream flow while keeping the downstream flow in the caller's context.&quot;</em></li><li>So the builder and every operator above the call move. Every operator below it, and <code>collect</code> itself, stay put — a <code>viewModelScope</code> collector keeps <code>Dispatchers.Main</code>, and UI updates inside <code>collect</code> stay safe.</li><li>It exists because a <code>flow { }</code> builder is not allowed to change its own context. Wrapping an <code>emit()</code> in <code>withContext(Dispatchers.IO)</code> fails at runtime; <code>flowOn</code> is the sanctioned way, and <code>channelFlow</code> is the other.</li><li>Chain two of them and each governs the segment above itself, up to the next one. The lower call does not undo the higher one.</li></ul>",
            referenceLinks: [{ title: "Flows — change the coroutine context with flowOn", url: "https://kotlinlang.org/docs/coroutines-flow.html#change-the-coroutine-context-of-a-cold-flow-with-flowon" }],
...
// downstream .collect { } still runs on the caller's (e.g. Main) dispatcher`,
        ...
        },
        {
            ...
            referenceLinks: [{ title: "Asynchronous Flow - Kotlin", url: "https://kotlinlang.org/docs/coroutines-flow.html" }],
        ...
        },
        {
            ...
            referenceLinks: [{ title: "Asynchronous Flow - Kotlin", url: "https://kotlinlang.org/docs/coroutines-flow-operators.html#terminal-operators" }],
        ...
        },
        {
            ...
            referenceLinks: [{ title: "Flows are cold - Kotlin", url: "https://kotlinlang.org/docs/coroutines-flow.html#cold-flows" }, { title: "StateFlow and SharedFlow", url: "https://developer.android.com/kotlin/flow/stateflow-and-sharedflow" }],
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 StateFlow is a value you can observe. SharedFlow is an event stream you can replay.</strong> Both are hot — they run whether or not anyone is collecting.</p><ul><li><strong>StateFlow</strong> always holds a current value in <code>.value</code>, hands it to every new collector, and skips a repeat of the value it already has. That skipping is <strong>conflation</strong> plus distinct-until-changed.</li><li><strong>SharedFlow</strong> holds no current value. Its <strong>replay</strong> cache decides how many past values a new collector receives, and the usual answer is zero.</li><li>So <code>StateFlow</code> behaves like a <code>SharedFlow(replay = 1)</code> that also conflates and drops duplicates.</li><li>Use <code>StateFlow</code> for state a screen renders — a ViewModel's <code>uiState</code>. Use <code>SharedFlow</code> for things that happen once — a snackbar, a navigation event — where replaying the latest value would fire it a second time on rotation.</li><li><code>MutableSharedFlow(replay = 0, extraBufferCapacity = 1, onBufferOverflow = DROP_OLDEST)</code> is the usual one-shot event channel.</li></ul><p><strong>⚖️ Against LiveData</strong></p><ul><li>Both are hot, and both have a lifecycle-aware collector — <code>collectAsStateWithLifecycle</code> against <code>observe</code>. <code>StateFlow</code> also lets you pick the dispatcher, test without an instrumentation runtime, and use every Flow operator on the way through.</li></ul>",
            referenceLinks: [{ title: "StateFlow and SharedFlow", url: "https://developer.android.com/kotlin/flow/stateflow-and-sharedflow" }, { title: "Flows — hot flows", url: "https://kotlinlang.org/docs/coroutines-flow.html#hot-flows" }],
...
fun onQueryChanged(text: String) {
    query.value = text
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 <code>catch</code> handles what goes wrong upstream of it. A <code>try</code>/<code>catch</code> around <code>collect</code> handles everything, the collector's own lambda included.</strong></p><ul><li><code>catch { }</code> sees exceptions from the builder and from operators above it. The KDoc: it <em>&quot;is transparent to exceptions that occur in downstream flow&quot;</em> — an operator placed below it, or the <code>collect</code> lambda, is not its business.</li><li>That is why a <code>try</code>/<code>catch</code> around <code>collect { }</code> is not redundant with it. It is the only one of the two that catches a failure in the collector body.</li><li><code>catch</code> can <code>emit()</code> a fallback value, turning a failure into a recovery emission. It can also rethrow selectively — <code>if (e !is IOException) throw e</code> — and anything thrown from its own action continues downstream to the next <code>catch</code>.</li><li>It <strong>does not catch cancellation</strong>: <em>&quot;does not catch exceptions that are thrown to cancel the flow&quot;</em>. Note the mechanism, because it is usually stated wrongly — <code>catch</code> never receives the <code>CancellationException</code> at all, rather than catching it and rethrowing.</li><li>To retry rather than recover, use <code>retryWhen</code>. The KDoc recommends it over falling back with <code>emitAll</code>, which builds an ever-growing stack of suspending calls.</li></ul>",
            referenceLinks: [{ title: "Flows — handle exceptions with catch", url: "https://kotlinlang.org/docs/coroutines-flow.html#use-the-catch-operator-to-handle-upstream-exceptions" }, { title: "catch — kotlinx.coroutines API", url: "https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines.flow/catch.html" }],
