const javaData = {
    ...
    questions: [
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 An abstract class is a partly built type you extend. An interface is a capability you promise to provide. You get one of the first and as many of the second as you like.</strong></p><table><thead><tr><th>Aspect</th><th>Abstract class</th><th>Interface</th></tr></thead><tbody><tr><td>How many</td><td>Extend exactly one</td><td>Implement as many as you like</td></tr><tr><td>State</td><td>Instance fields, any access modifier</td><td>Constants only — fields are implicitly <code>public static final</code></td></tr><tr><td>Constructors</td><td>Yes, reached from a subclass through <code>super()</code></td><td>None</td></tr><tr><td>Method bodies</td><td>Abstract and implemented methods, mixed freely</td><td>Abstract, plus <code>default</code> and <code>static</code> since Java 8</td></tr><tr><td>Access modifiers</td><td><code>public</code>, <code>protected</code>, <code>private</code></td><td>Implicitly <code>public</code>, or <code>private</code> for a helper since Java 9</td></tr><tr><td>Reach for it when</td><td>Related types share state and code</td><td>Unrelated types share an ability</td></tr></tbody></table><ul><li><strong>The diamond problem</strong> is why the counts differ. Two superclasses could supply the same field twice with different values, so Java forbids it. Two interfaces cannot, because they carry no state.</li><li>Default methods reintroduced a smaller version of the same conflict, and Java makes you settle it by hand: implement the method and pick a side with <code>Interface.super.method()</code>.</li><li><strong>Rule of thumb</strong> — abstract class for IS-A, where subclasses share implementation. Interface for CAN-DO, the way <code>Comparable</code> and <code>Serializable</code> are adopted by types with nothing else in common.</li></ul><p><strong>🎯 Interview tip:</strong> Since Java 8 gave interfaces method bodies, the line is thinner than it was. The distinction that survives is state — an interface still cannot hold any.</p>",
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 The GC frees heap objects that nothing can reach any more. You never call <code>free()</code>; reachability decides.</strong></p><ul><li>An object is alive if a chain of references reaches it from a <strong>GC root</strong> — a local variable on a stack, a static field, an active thread, a JNI reference. Everything else is garbage, whether or not you meant it to be.</li><li>The <strong>generational hypothesis</strong> is that most objects die young, so the heap is split. The <strong>Young Generation</strong> (Eden plus two Survivor spaces) is collected often and cheaply; survivors are promoted to the <strong>Old Generation</strong>, which is collected rarely and expensively.</li><li>The algorithm is <strong>mark, sweep, and often compact</strong>: mark what is reachable from the roots, reclaim what is not, then slide the survivors together so the free space is contiguous.</li><li><strong>Stop-the-world</strong> pauses are the cost. Some phases must halt every application thread to walk the object graph safely, and the whole design of a modern collector is about shrinking that window.</li><li>Four collectors ship with HotSpot. <code>Serial</code> for one processor and small heaps, <code>Parallel</code> for throughput, <code>G1</code> — region-based, and <em>&quot;selected by default on most hardware and operating system configurations&quot;</em> — and <code>ZGC</code>, which does its expensive work concurrently for <em>&quot;max pause times of a few milliseconds, but at the cost of some throughput&quot;</em>.</li><li><strong>What it will not do for you.</strong> It does not touch the stack — frames pop when a method returns. And it cannot free an object you can still reach but never use, which is exactly what a forgotten static reference is.</li></ul><p><strong>🎯 Interview tip:</strong> <code>System.gc()</code> is a hint, not a command. Saying so unprompted is a cheap way to show you know where the boundary of your control is.</p>",
            referenceLinks: [{ title: "Oracle: Garbage Collection Tuning Guide", url: "https://docs.oracle.com/en/java/javase/17/gctuning/introduction-garbage-collection-tuning.html" }, { title: "Oracle: Available Collectors", url: "https://docs.oracle.com/en/java/javase/17/gctuning/available-collectors.html" }],
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 A pool of worker threads that pull tasks off a queue, so you pay for a thread once instead of once per task.</strong> It is what the <code>Executors</code> factory methods actually build.</p><ul><li>Four settings do the shaping: <code>corePoolSize</code> (threads kept even when idle), <code>maximumPoolSize</code> (the hard cap), <code>keepAliveTime</code> (how long a thread above core size survives idle), and the <code>BlockingQueue</code> that holds waiting tasks.</li><li><strong>Where a submitted task goes</strong>, in order: start a new thread if the pool is below <code>corePoolSize</code>; otherwise queue it; if the queue is full, start a new thread up to <code>maximumPoolSize</code>; if that is full too, hand it to the <code>RejectedExecutionHandler</code>.</li><li>The surprise in that order is the middle step. <strong>An unbounded queue means <code>maximumPoolSize</code> never applies</strong>, because the queue never fills — a pool sized 4 to 64 with a <code>LinkedBlockingQueue</code> is a pool of 4.</li><li><strong>Sizing.</strong> CPU-bound work wants roughly <code>Runtime.getRuntime().availableProcessors()</code> threads. I/O-bound work wants more, because those threads spend their time blocked rather than computing.</li><li><strong>Shut it down.</strong> <code>shutdown()</code> finishes what is queued and refuses new work; <code>shutdownNow()</code> tries to cancel what is running. A pool nobody shuts down keeps the JVM alive.</li><li><strong>On Android</strong> this is what <code>AsyncTask</code> ran on and what a custom background <code>Executor</code> still is, though new code reaches for coroutines and a dispatcher instead.</li></ul><p><strong>🎯 Interview tip:</strong> Learn the four rejection policies — <code>AbortPolicy</code> (throws, and is the default), <code>CallerRunsPolicy</code>, <code>DiscardPolicy</code>, <code>DiscardOldestPolicy</code>. &quot;What happens when the queue and the pool are both full&quot; is a standard follow-up.</p>",
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 The <code>java.util.concurrent.atomic</code> classes give you read-modify-write in one indivisible step, using the CPU's compare-and-swap instruction instead of a lock.</strong> No thread ever blocks.</p><ul><li><code>get()</code> — a <code>volatile</code> read. You see the latest write from any thread.</li><li><code>set(v)</code> — a <code>volatile</code> write. Every other thread sees it.</li><li><code>lazySet(v)</code> — a <strong>release</strong> store, not a lazy one despite the name. Writes made before it cannot be reordered after it, but it does not carry the full fence a <code>volatile</code> write does, so another thread may not see it promptly. Cheaper than <code>set()</code>, and the classic use is nulling a reference you are about to drop.</li><li><code>compareAndSet(expected, update)</code> — write <code>update</code> only if the value is still <code>expected</code>, and report which happened. Every lock-free algorithm is built on this.</li><li><code>weakCompareAndSet</code> — the same, except it may <strong>fail spuriously</strong> when the value did match. That is why it belongs inside a retry loop and never in a one-shot check.</li><li>Careful with that last one: it was <strong>deprecated in Java 9</strong>, not renamed. The javadoc's reason is the name — <em>&quot;This method has plain memory effects but the method name implies volatile memory effects&quot;</em> — and it directs you to the explicit <code>weakCompareAndSetPlain</code>. There are now four variants, <code>Plain</code>, <code>Acquire</code>, <code>Release</code> and <code>Volatile</code>, so the memory ordering is stated rather than guessed.</li></ul><p><strong>🎯 Interview tip:</strong> The CAS retry loop — <code>do { current = get(); } while (!compareAndSet(current, current + 1));</code> — is how <code>incrementAndGet()</code> works underneath. Being able to write it is the point of the question.</p>",
