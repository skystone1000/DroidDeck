const otherTopicsData = {
    ...
    "questions": [
        ...
        },
        {
            ...
            "answer": "<p><strong>🔑 A processor reads the annotations in your source at compile time and writes more code from them.</strong> That is why Room, Hilt and Moshi cost you nothing at runtime — the work already happened, and there is no reflection.</p><p><strong>⚙️ kapt against KSP</strong></p><ul><li><strong>kapt</strong> exists so Java annotation processors can run on Kotlin. It gets there by generating Java stubs from your Kotlin files for the processors to read — and the Android guide is blunt that <em>&quot;this stub generation is an expensive operation and has a significant impact on build speed&quot;</em>.</li><li><strong>KSP</strong> analyses Kotlin directly instead, which the guide puts at <em>&quot;up to 2x faster&quot;</em>. It also understands Kotlin's own constructs — nullability, extension functions, declaration-site variance — rather than seeing them through a Java-shaped stub.</li><li><strong>kapt is in maintenance mode</strong>, and Google's own recommendation is to migrate every processor that supports KSP. You can run both side by side and move module by module.</li><li>The trade KSP makes: a processor can see declarations but not expressions or statements, and it cannot modify your source. Enough for codegen, not for a compiler plugin.</li></ul><p><strong>✅ Who uses it</strong></p><ul><li><strong>Room</strong> generates DAO implementations, <strong>Hilt</strong> and <strong>Dagger</strong> generate the dependency graph, <strong>Moshi</strong> generates JSON adapters. All three ship KSP processors.</li></ul><p><strong>🎯 Interview tip:</strong> &quot;Replace kapt with KSP&quot; is the right answer to &quot;how would you speed up this build&quot;, and saying kapt is in maintenance mode is what makes it sound current rather than remembered.</p>"
        },
        {
            "referenceLinks": [
                {
        ...
        },
        {
            ...
            "answer": "<p><strong>🔑 <code>AlarmManager</code>, not WorkManager. WorkManager is for work that may be deferred; an exact time is the one thing it does not promise.</strong></p><ul><li><code>setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, triggerAtMillis, pendingIntent)</code> fires at a wall-clock time and survives <strong>Doze</strong>.</li><li>The <code>PendingIntent</code> wakes a <code>BroadcastReceiver</code>, and the receiver is what builds and posts the <code>Notification</code> through <code>NotificationManagerCompat.notify()</code>. The alarm itself displays nothing.</li></ul><p><strong>⚠️ The permission, which changed twice</strong></p><ul><li>Targeting <strong>Android 12 (API 31)</strong> or higher, exact alarms need the <em>&quot;Alarms &amp; reminders&quot;</em> special app access, declared as <code>SCHEDULE_EXACT_ALARM</code>.</li><li>Targeting <strong>Android 13 (API 33)</strong> or higher, you choose between two. <code>SCHEDULE_EXACT_ALARM</code> is <strong>not pre-granted to fresh installs</strong> and the user must approve it. <code>USE_EXACT_ALARM</code> is granted automatically and cannot be revoked, but it is for alarm-clock and calendar-style apps only and is subject to a Play policy.</li><li>Because the user or the system can revoke <code>SCHEDULE_EXACT_ALARM</code>, call <code>canScheduleExactAlarms()</code> before every schedule and fall back to an inexact alarm when it returns false. Revocation stops your app and <strong>cancels every future exact alarm you had set</strong>.</li><li>Listen for <code>ACTION_SCHEDULE_EXACT_ALARM_PERMISSION_STATE_CHANGED</code> and reschedule when it arrives, the same way you would on <code>BOOT_COMPLETED</code>.</li></ul><p><strong>⚠️ Two more ways it silently fails</strong></p><ul><li>Alarms do not survive a reboot. Re-register on <code>BOOT_COMPLETED</code> or they are simply gone.</li><li>On Android 13 and above, without runtime <code>POST_NOTIFICATIONS</code> the alarm fires, the receiver runs, and nothing appears.</li></ul>"
        }
    ]
};
