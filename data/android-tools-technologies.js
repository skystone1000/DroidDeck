const androidToolsTechnologiesData = {
    ...
    questions: [
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 Android assumed 4&nbsp;KB memory pages for its whole history. From Android 15, AOSP supports devices configured for 16&nbsp;KB pages, and native libraries built against the old assumption will not load on them.</strong></p><ul><li>It is a <strong>Play requirement, with a date</strong>. Apps targeting Android 15 (API 35) or higher must support 16&nbsp;KB pages on 64-bit devices, and <strong>from 1 February 2027</strong> an update that does not will be refused at release.</li><li>Only native code is affected — but read that carefully. The guide's condition is <em>&quot;if your app uses any NDK libraries, either directly or indirectly through an SDK&quot;</em>, and the indirect case is the one that surprises people. A pure-Kotlin app that pulls in one analytics SDK with a bundled <code>.so</code> is in scope.</li><li><strong>Find out</strong> by looking for a <code>lib</code> folder in the APK. No shared objects means no native code and nothing to do. Android Studio's lint and the APK Analyzer both flag unaligned libraries directly.</li><li><strong>Fix it</strong> by rebuilding with 16&nbsp;KB ELF alignment — <code>-Wl,-z,max-page-size=16384</code> and <code>-Wl,-z,common-page-size=16384</code> — set through <code>Android.mk</code> for ndk-build or <code>target_link_options</code> for CMake.</li><li>Devices with 16&nbsp;KB pages use slightly more memory and get it back in performance: fewer page-table entries, faster launches, better behaviour under memory pressure.</li></ul><p><strong>🎯 Interview tip:</strong> The date is the answer. &quot;Targeting API 35 or higher, 64-bit, and blocked from releasing updates from February 2027&quot; is a compliance fact; &quot;newer devices use bigger pages&quot; is trivia.</p>",
        ...
        },
        {
            ...
            answer: "<p><strong>🔑 Firebase is Google's Backend-as-a-Service: hosted pieces of backend your app talks to directly through SDKs, so you do not build or operate them.</strong> The trade you are making is speed now against lock-in later.</p><table><thead><tr><th>Service</th><th>What it removes</th></tr></thead><tbody><tr><td><strong>Cloud Firestore</strong> / <strong>Realtime Database</strong></td><td>Hosted NoSQL with offline caching and realtime listeners that push changes to connected clients</td></tr><tr><td><strong>Authentication</strong></td><td>Email, phone and federated sign-in (Google, Apple, Facebook), and the token handling behind it</td></tr><tr><td><strong>Cloud Messaging (FCM)</strong></td><td>Push delivery — on Android, the only supported route to a backgrounded app</td></tr><tr><td><strong>Crashlytics</strong></td><td>Crash and ANR reporting, deobfuscated once you upload the mapping file</td></tr><tr><td><strong>Remote Config</strong></td><td>Feature flags and staged rollouts without shipping a release</td></tr><tr><td><strong>Analytics</strong></td><td>Event funnels, and the audiences Remote Config and messaging campaigns target</td></tr></tbody></table><p><strong>⚖️ The cost side</strong></p><ul><li>The query model is much weaker than SQL — no joins, limited aggregation — and you cannot migrate off it cheaply once the data shape has followed it.</li><li>Firestore bills per document read, not per unit of compute. An unbounded listener on a large collection is a <strong>billing</strong> bug that looks like a performance one.</li><li>Security rules are the <em>entire</em> authorization layer between any client and your data. Your config is publicly visible, so a permissive rule is an open database.</li><li>The SDK brings in Google Play Services, which costs APK size and rules out markets where Play Services is not available.</li></ul><p><strong>🎯 Interview tip:</strong> Name one service and the exact problem it removes. &quot;FCM, because Android will not let you hold a socket open in the background, so push has to go through an OS-level channel&quot; beats reciting the product catalogue.</p>",
